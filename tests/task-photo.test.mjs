...
const result=await build({entryPoints:['app/api/task-photo/route.ts'],bundle:true,platform:'node',format:'esm',write:false,plugins:[{name:'route-mocks',setup(b){b.onResolve({filter:/lib\/server$/},()=>({path:'server',namespace:'mock'}));b.onResolve({filter:/lib\/telegram$/},()=>({path:'telegram',namespace:'mock'}));b.onLoad({filter:/.*/,namespace:'mock'},args=>({contents:args.path==='server'?'export async function identity(){return globalThis.__photoIdentity;}':"export async function telegramFile(id){globalThis.__fileId=id;if(globalThis.__photoError)throw Object.assign(new Error('telegram failed'),{status:globalThis.__photoError});return new Response(new Uint8Array([1,2,3]),{headers:{'Content-Type':'image/jpeg'}});}"}));}}]});
...
test('task photo upstream failures are reported as gateway errors, not authentication errors',async()=>{globalThis.__photoIdentity={s:state,m:{id:'m',name:'УМ',email:'',role:'manager',branches:['b1'],active:true}};globalThis.__photoError=502;try{const r=await api.GET(new Request('https://app.test/api/task-photo?shift=s1&task=hall-dust&index=0'));assert.equal(r.status,502);}finally{globalThis.__photoError=0;}});
